    ...
    create or replace table `stately-node-363801`.`network`.`cleaned_data_silver`
...
WITH cleaned_data_silver AS (
  SELECT 
  SUBSTR(GENERATE_UUID(), 1, 12) AS primary_key,
  *
  FROM 
    `stately-node-363801`.`network`.`network_raw_bronze`
